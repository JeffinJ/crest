...
import { CreateUserConnectionSchema, CreateUserConnectionSchemaType, DeleteUserConnectionSchema, EditUserConnectionSchema, EditUserConnectionSchemaType } from "@zod-schemas/user-connections";
...
export const deleteUserConnectionAction = actionClient
    .metadata({ action: 'deleteUserConnection' })
    ...
    .action(async ({ parsedInput }: { parsedInput: number }) => {
        ...
        if (!authData) return redirect('/signin');
        const JWT = authData.value;
        const responseData = await fetch(process.env.NEXT_PUBLIC_CREST_AUTH_CENTER_URL + '/auth/connection/' + parsedInput, {
            method: 'DELETE',
            headers: {
                'Content-Type': 'application/json',
                Authorization: `Bearer ${JWT}`,
        ...
        return { success: 'Connection deleted', data: await responseData.json() };
    })